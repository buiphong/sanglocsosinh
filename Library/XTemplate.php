...
class XTemplate {
    ...
    /**
     * @var Giá trị đối chiếu có sử dụng function
     */
    public $_function = array('thumb:','CutString:');
	...
	public function __construct($file = '', $tpldir = '', $files = null, $mainblock = 'main', $autosetup = true) {
        ...
        //Load language
        if(MULTI_LANGUAGE && empty($_SESSION['langcode']))
        {
            //Bổ sung language - phongbui
        ...
        if(!PTRegistry::isRegistered('webConfig'));
        {
            Helper::loadModule('ConfigCP');
            PTRegistry::set('webConfig', Models_ConfigValue::getConfig('website_address,website_email,website_phone,
            website_fax,website_hotline,config_logo,conf_title,conf_description,conf_keywords,conf_seo_title,conf_seo_alt,conf_facebook_fanpage'));
        }
	}
    ...
    /**
     * change new file view
     * @author PhongBui
     */
    public function changeFile($file, $tpldir = '', $files = null, $mainblock = 'main', $autosetup = true, $tag_start = '{', $tag_end = '}') {
        $this->filename = $file;
	...
	public function parse ($bname) {
		$this->vars['pt_language'] = $this->language;
		$this->vars['appPath'] = Url::getApplicationUrl();
        $this->vars['web_config'] = PTRegistry::get('webConfig');
		...
		foreach ($var_array as $k => $v) {
			...
			$sub = explode('.', $v);
            //Check for string
            $fstr = substr($sub[0], 0, 2);
			if ($sub[0] == '_BLOCK_' && $fstr != 'AG') {
				...
				$var = str_replace('\\|', '|', $var);
                //Xử lý comment
                foreach($this->_function as $s)
                {
                    if(strpos($comments, $s) !== false)
                    {
                        $var = $this->parseFunction($s, $comments, $var);
                        break;
                    }
                }
    ...
    */
    ...
    {
        //Check key
        $fn = base64_encode($this->filename . 'js') . '.js';
        $fnPath = ROOT_PATH . DIRECTORY_SEPARATOR . COMBINE_ASSETS_DIR . DIRECTORY_SEPARATOR . $fn;
        if(!file_exists($fnPath) || DEBUG)
        {
            $content = '';
            $appendLink = '';
            foreach($this->blocks as $k => $t)
            {
                if($k == 'main.loadScript')
                {
                    $t = trim($t);
                    $arr = explode(',', $t);
                    if(DEBUG)
                    {
                        ...
                        {
                            $a = trim($a);
                            $appendLink .= '<script type="text/javascript" src="'.Url::getApplicationUrl() .'/'. $a .'"></script>';
                        }
                    }
                    else
                    {
                        if(COMPRESS_JS)
                            $content = Compress::CompressJs($arr);
                        else
                        {
                            foreach($arr as $a)
                            {
                                $a = Url::getAppDir().trim($a);
                                //read content file, write to one
                                if(!file_exists($a))
                                    throw new Exception('Không tìm thấy tập tin: ' . $a);
                                $content .= "\n". file_get_contents($a);
                            }
                        }
                    }
            ...
            if(!empty($content))
            {
                file_put_contents($fnPath, $content);
            }
        }
        //include file
        if(DEBUG)
    ...
    {
        //Check key
        $fn = base64_encode($this->filename . 'css') . '.css';
        $fnPath = ROOT_PATH . DIRECTORY_SEPARATOR . COMBINE_ASSETS_DIR . DIRECTORY_SEPARATOR . $fn;
        if(!file_exists($fnPath) || DEBUG)
        {
            $content = '';
            ...
            foreach($this->blocks as $k => $t)
            {
                if($k == 'main.loadCss')
                {
                    ...
                    if(DEBUG)
                    {
                        ...
                        {
                            $a = trim($a);
                            $appendLink .= '<link rel="stylesheet" href="'.Url::getApplicationUrl() .'/'. $a .'"/>';
                        }
                    }
                    else
                    {
                        foreach($arr as $a)
                        {
                            $a = trim($a);
                            //read content file, write to one
                            $cssContent = @file_get_contents(Url::getAppDir() . DIRECTORY_SEPARATOR . $a);
                            //Change url image
                            $filePath = explode('/', $a);
                            unset($filePath[count($filePath) - 1]);
                            preg_match_all('/url\((.*?)\)/i', $cssContent, $imgs);
                            $replaced = array();
                            foreach($imgs as $img)
                            {
                                $img = str_replace('"', '', $img);
                                ...
                                foreach($img as $i)
                                {
                                    ...
                                    if(empty($url['scheme']))
                                    {
                                        ...
                                        $f = Url::getAppDir() . DIRECTORY_SEPARATOR . implode(DIRECTORY_SEPARATOR, $path) . DIRECTORY_SEPARATOR . implode(DIRECTORY_SEPARATOR, $p);
                                        if(file_exists($f) && filesize($f) < 10240)
                                            $imgLink = Url::base64EncodeImage($f);
                                        else
                                            $imgLink = Url::getApplicationUrl() . '/' . implode('/', $path) . '/' . implode('/', $p);
                                        if(!in_array($i, $replaced))
                                        {
                                            $cssContent = str_replace($i,$imgLink, $cssContent);
                                            $replaced[] = $i;
                                        }
                                    }
                                }
            ...
            if(!empty($content))
            {
                file_put_contents($fnPath, $content);
                if(COMPRESS_CSS)
                    file_put_contents($fnPath, Compress::CompressCss(array($fnPath)));
            }
        }
        //include file
        if(DEBUG)
            $this->parsed_blocks['main.loadCss'] = $appendLink;
        else
            $this->parsed_blocks['main.loadCss'] = '<link rel="stylesheet" href="'.Url::getApplicationUrl() .'/'.str_replace(DIRECTORY_SEPARATOR, '/',COMBINE_ASSETS_DIR) . '/'.$fn.'"/>';
    }

    /**
     * Xử lý function bổ sung
     * @author phongbui
     */
    public function parseFunction($func, $comment, $val)
    {
        if($val)
        {
            switch($func)
                ...
                case 'thumb:':
                    ...
                    if($func[1])
                    {
                        ...
                        if(count($func) > 2)
                        {
                            $option = array();
                            //Crop
                            if(isset($func[2]) && $func[2] == 1)
                                $option['crop'] = 'auto';
                            switch($func[2])
                            {
                                case 'tl':
                                    $option['crop'] = 'crop_top';
                                    break;
                                case 'tm':
                                    $option['crop'] = 'crop_tm';
                                    break;
                            }
                            $val = Url::getThumb($val, $func[0], $func[1], $option);
                        }
                        elseif(count($func) == 2)
                            $val = Url::thumbnail($val, $func[0], $func[1]);
                        elseif(count($func) == 1)
                            $val = Url::thumbnail($val, $func[0], '');
                    }
                    break;
                case "CutString:":
                    $func = explode(':', $comment);
                    ...
                    {
                        $number = $func[1];
                        $val = String::get_num_word($val, $number);
                    }
                    break;
            }
        }
