...
class Uri
{
	...
	{
		$basePath = explode('/', $this->getBasePath());
		$arrUri = explode('?', $this->getRequestUri());
        $requestUri = empty($arrUri[0])? '':explode('/', $arrUri[0]);
        $params = empty($arrUri[1])? array():$arrUri[1];
		...
		return array('request' => $requestUri, 'params' => $params);
	}
	
	/**
	 * Phân tích URI loại bỏ BASEPATH và trả về mảng - Dành cho rest
	 * @param void
	 * @return array
	 */
	public function restParseRequestUri()
	{
		$basePath = explode('/', $this->getBasePath());
		$requestUri = explode('/', $this->getRequestUri());
		$requestUri = array_slice($requestUri, sizeof($basePath),
				sizeof($requestUri) - sizeof($basePath));
		return $requestUri;
	}
