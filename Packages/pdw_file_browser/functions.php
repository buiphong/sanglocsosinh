 ...
 * @return 
 */
function renderTree($dirs, $rootpath, $selectedPath = ''){
	$html = "\t\t<ul>\n";
	
	foreach ($dirs as $key => $value) {
		if ($rootpath.$key . '/' == $selectedPath) {
			$class = 'selected';
		}
		else 
			$class = '';
		$html .= "\t\t\t<li class='$class'><a href=\"".$rootpath.$key."/".'" class="folder">'.$key.'</a>';
		
		if (count($dirs[$key]) > 0) {
			$html .= "\n" . renderTree($dirs[$key], $rootpath.$key."/", $selectedPath) . "\t\t";
		}
